...
Base.@propagate_inbounds function update_velocity!(b1, b2, Δt)
    # Fastest implementations first calculate Δpos for all body pairs
    # and then use that to calculate mag for all pairs. Possibly
    # better cache-locality is maintained by doing so instead of
    # just directly updating velocities all at once
    Δpos = b1.pos .- b2.pos
    d² = sum(Δpos .* Δpos)
    ...
    d = √d²
    mag = Δt / (d² * d)
    # Separating mag1/mag2 calculation into its own line and variable
    # declaration rather than calculating it inline below generates
    # faster code for some reason
    mag1 = -b2.m * mag
    mag2 = b1.m * mag
...
function next!(bodies, Δt)
    ...
    @inbounds for i in 1:length(bodies), j in i+1:length(bodies)
        update_velocity!(bodies[i], bodies[j], Δt)
    end#for
